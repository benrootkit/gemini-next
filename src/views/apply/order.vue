...
let selectedSourceRows = [] as ISource[];
...
const postOrder = () => {
  ...
  formRef.value
      ...
      .then(async () => {
        ...
        /* batch */
        wrapper.source_id_list = [];
        selectedSourceRows.forEach((item) => {
          wrapper.source_id_list = wrapper.source_id_list.concat(item.source_id);
        });

        wrapper.source_list = [];
        selectedSourceRows.forEach((item) => {
          wrapper.source_list = wrapper.source_list.concat(item.source);
        });
...
const onSourceSelectChange = (selectedRowKeys, selectedRows) => {
  ...
  selectedSources.value = selectedRowKeys;
  selectedSourceRows = selectedRows;
};

const rowSourceSelection = ref({
